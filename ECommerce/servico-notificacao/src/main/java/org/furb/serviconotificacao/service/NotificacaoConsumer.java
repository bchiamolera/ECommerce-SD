...
public class NotificacaoConsumer {
    ...
    public static final String RK_PEDIDO_FALHA = "pedido.falha";
    ...
    public NotificacaoConsumer(RabbitTemplate rabbitTemplate, EmailService emailService) {
        this.rabbitTemplate = rabbitTemplate;
        this.emailService = emailService;
        this.objectMapper = new ObjectMapper();
    }

    @RabbitListener(queues = FILA_NOTIFICACAO)
    ...
    private long getRetryCount(List<Map<String, ?>> xDeathHeader) {
        return Optional.ofNullable(xDeathHeader)
                .flatMap(headers -> headers.stream()
                        .filter(header -> FILA_RETRY_NOTIFICACAO.equals(header.get("queue")))
                        .findFirst()
                )
     ...
     */
    @RabbitListener(queues = FILA_DLQ_NOTIFICACAO)
